...
""".format(invoke=Cli.__INVOCATION_CMD),
        ...
        if not hasattr(self, args.command):
            print(
                "[ERROR]\t'{command}' is not a {invoke} command\n".format(
                    command=args.command, invoke=Cli.__INVOCATION_CMD
                )
            )
            parser.print_help()
            exit(1)
    ...
    def version(self):
        print(
            "pyPS4Controller {} (Python{})\n".format(
                pkg_resources.require("pyPS4Controller")[0].version, sys.version_info[0]
            )
        )
